...
$bodyLine = (Select-String -Path $mainFile -Pattern '^<body ' | Select-Object -First 1).LineNumber
$scriptLine = (Select-String -Path $mainFile -Pattern '^\s*<script>\s*$' | Where-Object { $_.LineNumber -gt $bodyLine } | Select-Object -First 1).LineNumber

if (-not $bodyLine -or -not $scriptLine) {
  throw "Could not find expected body/script boundaries in sendlovetocuba.html"
}
...
Set-Content -Path $mainFile -Value $out
